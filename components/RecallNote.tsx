...
export default function RecallNote({note, handle} : {note : TDBNoteEntry, handle : () => void}){
    ...
    <>
            <div className='relative top-0 bg-blue-950'> 
                <div className="absolute left-29 sm:left-56.5 top-10 rounded-2xl w-[55px] h-[110px]">
          ...
          <div className="bg-blue-400 w-[350px] sm:w-[600px] max-h-[800px] rounded-4xl mt-8 p-3 overflow-auto" onClick={() => {if(quality != 6) return; setDrop(!drop)}}>
        ...
        : 
                <div className='center'>
                    <form className='border-2 border-blue-950 rounded-2xl flex flex-col justify-center items-center w-1/2 py-2' action={(e) => { if(![0, 1, 2, 3, 4, 5].includes(quality)) return; setQuality(-1); handle(); action(e);}}>
                        ...
                        <select id="recall" defaultValue={-1} name="recall" onChange={(e) => setQuality(Number(e.target.value))} className="mt-1 block w-[120px] text-xs px-1 py-2 border border-blue-950 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
