    ...
    return (
        ...
        <div key='input' className="flex flex-col justify-center items-center mt-60 w-[500px] h-[200px]">
                <div className="flex flex-col justify-center items-center">
                    <form action={(e) => { cleanUp(); saveNotes(e);}} className='w-[450px] bg-blue-400 rounded-4xl p-2 flex flex-col justify-center items-center'>
                        ...
                        <input ref={wordInputRef} key='userWord' className='text-blue-50 formInput mt-8 mb-3' type="text" name="word" defaultValue={word} onChange={(e) => setWord(e.target.value)} placeholder='Enter new word here...'/>
                        {note?.error && <p className='error'>{note.error}</p>}
    
                        <div className="flex justify-start">
                          <input key="audioInput" type="text" hidden name="audio" defaultValue={note?.sound}/>
                          {note?.parsedNote ? <p className='text-start'>Pronounciation: <AudioPlayer src={note?.sound} ></AudioPlayer></p> : <></>}
                        </div>
                        {generate && 
                        <div>
                        ...
                        {generate && 
                        <div>
                             <textarea  rows={5} cols={40} placeholder='Notes will be generated here...' className='mt-2 max-h-50' name='generatedNotes' key="genNotes" value={(note.error === undefined ? note?.parsedNote : "")}
