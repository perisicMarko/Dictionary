...
export default function UserInput(){
    ...
    return (
        ...
        :
        <motion.div initial='hidden' animate='show' variants={containerVariants} key='help' className="w-full sm:w-[400px] md:w-[500px] xl:w-[700px] xl:h-[400px] rounded-4xl mt-10 xl:mt-50 p-3"  id='help'>
          <motion.div variants={itemVariants} className='center rounded-t-2xl mb-5'>
          <h1 className="hover:underline cursor-pointer bg-slate-800 text-white w-1/4 text-center rounded-2xl" onClick={() => setHelp(!help)}><b>{help ? 'Go back' : 'Need hlep?'}</b></h1> 
          </motion.div>
          <motion.div variants={itemVariants}>
            <Image className="inline-block rounded-2xl border-2 border-slate-800 mr-3" width={350} height={280} src='/wordInput.png' alt='picture of word input'></Image>
            <motion.p variants={{hidden: {opacity: 0, x:100}, show: {opacity: 1, x: 0}}} className="inline-block bg-slate-800 text-white rounded-2xl p-1 sm:w-[200px]">     
                Input the word you would like to remember, then click the &quot;Generate&quot; button. 
            </motion.p>
          </motion.div>
          <motion.div variants={itemVariants} className='mt-5'>
            <Image className="inline-block rounded-2xl border-2 border-slate-800 mr-3" width={350} height={250} src='/generateNotes.png' alt='generate notes'></Image>
            <motion.p variants={{hidden: {opacity: 0, x:100}, show: {opacity: 1, x: 0}}} className='inline-block bg-slate-800 text-white rounded-2xl p-1 sm:w-[200px]'>
