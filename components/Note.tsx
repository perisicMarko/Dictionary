...
function Note({prop, historyNote, handle} : {prop : TDBNoteEntry, historyNote : boolean, handle : () => void}){
    ...
    return (
        ...
        ref={containerRef} 
        className="relative bg-slate-800 w-3/4 sm:w-[600px] max-h-[720px] sm:max-h-[800px] rounded-4xl mt-8 p-7" title={title} onClick={() => {setMenu(false); setDrop(!drop);}}>
            <div className="absolute flex flex-col items-center justify-center top-5 right-0 rounded-2xl w-1/6">
                {historyNote && <Image className="scale-75 hover:scale-90 cursor-pointer" title='options' src='/menu.svg' width={30} height={30} alt='menu icon' onClick={(e) => {e.stopPropagation(); setMenu(!menu);}}></Image>}
                {menu && historyNote && 
                <motion.div initial='hidden' animate='show' variants={containerVariants} className='bg-white/80 z-10 rounded-2xl p-1'>
                    ...
                    <motion.form variants={itemVariants} className="center" action={async (e) => {setMenu(!menu); await backToRecallSystem(e); handle();}}>
                        <input type="text" name='noteId' defaultValue={note.id} hidden/>
                        <button type='submit' className='text-center text-xs text-slate-800 cursor-pointer hover:scale-110' onClick={(e) => e.stopPropagation()}>learn<br/>again</button>
                    </motion.form>
                </motion.div>
                }
            </div>
                
            <h2 className="text-white mb-3" title="word"><b>{note.word}</b></h2>
            <div className="flex flex-col justify-center items-center space-y-2">
                <AudioPlayer src={note.audio}></AudioPlayer>
                <button className="primaryBtn" onClick={() => {setDrop(!drop); setMenu(false);}}>Show notes<Image src={drop ? '/arrowUp.svg' : '/arrowDown.svg'} alt='arrow icon' width={20} height={20} className='ml-3 inline-block'></Image></button>
            </div>
            {drop && 
            <motion.div initial={{y:15, opacity: 0}} animate={{y: 0, opacity: 1}} className='p-4 overflow-auto h-[300px] xl:h-[400px]'>
